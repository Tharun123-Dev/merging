...
export default function Contracts() {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await rolesApi.post('/vendor-contracts', payload);
      fetchContracts();
            ...
            {filteredContracts.length > 0 ? filteredContracts.map((c) => (
              <div key={c.id} className="bg-slate-800/40 border border-slate-700/50 p-4 rounded-xl hover:bg-slate-800/60 hover:border-slate-600 transition-all group cursor-pointer" onClick={() => openView(c)}>
                ...
                <div className="flex gap-2 justify-end mt-3 pt-3 border-t border-slate-700/40">
                  ...
                  <button onClick={(e) => { e.stopPropagation(); handleDelete(c.id); }} className="btn-icon hover:text-rose-400 text-xs gap-1 flex items-center px-2 py-1 cursor-pointer" title="Delete">
            ...
            {expiringContract ? (
              <div className="bg-rose-500/10 border border-rose-500/20 p-4 rounded-xl">
                ...
                <button
                  ...
                  className="mt-3 text-xs bg-rose-500/20 text-rose-300 px-3 py-1.5 rounded-lg hover:bg-rose-500/30 transition-colors cursor-pointer"
                >
                  Renew Contract
                </button>
              </div>
      ...
      <Modal isOpen={isAddOpen} onClose={() => setIsAddOpen(false)} title="New Contract">
        <form className="space-y-4" onSubmit={handleAdd}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-1">Start Date</label>
              <input type="date" className="input-field text-slate-400" value={newContract.startDate} onChange={(e) => setNewContract({ ...newContract, startDate: e.target.value })} />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-1">Expiry Date</label>
              <input type="date" className="input-field text-slate-400" value={newContract.expires} onChange={(e) => setNewContract({ ...newContract, expires: e.target.value })} />
            </div>
            <div className="col-span-2">
              <label className="block text-sm font-medium text-slate-300 mb-1">Notes</label>
