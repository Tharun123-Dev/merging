  ...
  return (
    <Badge
      ...
      className={connected
        ...
        : 'bg-slate-100 text-slate-600 border-slate-200 hover:bg-slate-100 dark:bg-slate-800 dark:text-slate-400'
      }
    >
...
function Modal({ onClose, children }: { onClose: () => void; children: React.ReactNode }) {
  return (
    ...
    >
      <div className="bg-card border dark:border-slate-800 rounded-2xl w-full max-w-md shadow-2xl p-6 relative my-8 max-h-[90vh] overflow-y-auto">
        {children}
      </div>
    </div>
  );
...
export function IntegrationsPage() {
  ...
  const CARD_ACCENT = {
    ...
    apikey: 'from-slate-700 to-slate-900',
  };
    ...
    <div className="space-y-6 max-w-[1600px] mx-auto p-1 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 text-xs bg-card border dark:border-slate-800 p-2 rounded-xl shadow-sm">
            {statusLoading ? (
              <span className="flex items-center gap-1.5 text-slate-500"><Loader2 className="h-3.5 w-3.5 animate-spin" /> Checking…</span>
            ) : backendAvailable ? (
      ...
      <div className="relative overflow-hidden bg-gradient-to-r from-indigo-500/10 via-purple-500/10 to-pink-500/10 border border-indigo-100/50 dark:border-indigo-900/30 rounded-2xl p-5 shadow-sm">
        <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs font-medium text-slate-700 bg-card/90 dark:text-slate-300 backdrop-blur p-3.5 rounded-xl border dark:border-slate-800 shadow-sm">
            {[
              { icon: <CreditCard className="h-3 w-3" />, label: 'Cashfree Payment', color: 'amber' },
              null,
              { icon: <Webhook className="h-3 w-3" />, label: 'Webhook Trigger', color: 'slate' },
              null,
              { icon: <Zap className="h-3 w-3" />, label: 'Zapier Automation', color: 'purple' },
            ].map((item, i) =>
              item === null
                ? <ArrowRight key={i} className="h-3 w-3 text-slate-400" />
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        {/* 1 · Google Calendar */}
        <Card className="hover:shadow-md transition-shadow relative overflow-hidden flex flex-col justify-between border-slate-200/80">
          ...
          <CardContent className="pt-0 space-y-4">
            {status.google && googleEmail && (
              <div className="text-xs bg-slate-50 dark:bg-slate-900 border p-2.5 rounded-lg truncate text-slate-500">
                {googleEmail}
              </div>
            )}
        ...
        <Card className="hover:shadow-md transition-shadow relative overflow-hidden flex flex-col justify-between border-slate-200/80">
          ...
          <CardContent className="pt-0 space-y-4">
            {status.zoom && (
              <div className="text-xs bg-slate-50 dark:bg-slate-900 border p-2.5 rounded-lg flex justify-between">
                <span className="text-slate-500">{zoomMeetings.length} Scheduled Meeting{zoomMeetings.length !== 1 ? 's' : ''}</span>
        ...
        <Card className="hover:shadow-md transition-shadow relative overflow-hidden flex flex-col justify-between border-slate-200/80">
          ...
          <CardContent className="pt-0 space-y-4">
            {status.cashfree && cashfreeAppId && (
              <div className="text-xs bg-slate-50 dark:bg-slate-900 border p-2.5 rounded-lg flex justify-between">
                <span className="text-slate-500 truncate">App ID: {cashfreeAppId}</span>
        ...
        <Card className="hover:shadow-md transition-shadow relative overflow-hidden flex flex-col justify-between border-slate-200/80">
          ...
          <CardContent className="pt-0 space-y-4">
            <div className="space-y-2">
              <label className="text-[11px] font-semibold text-slate-500 block">Pixel ID / Conversion API Token</label>
              <div className="flex gap-2">
                <Input
        ...
        <Card className="hover:shadow-md transition-shadow relative overflow-hidden flex flex-col justify-between border-slate-200/80">
          ...
          <CardHeader className="pb-3">
            <div className="flex items-start justify-between">
              <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-slate-50 border border-slate-200 text-slate-600 dark:bg-background dark:border-slate-900">
          ...
          <CardContent className="pt-0 space-y-4">
            <div className="space-y-1">
              <div className="text-[11px] text-slate-400 font-semibold uppercase tracking-wider">Subscribed Events</div>
              <div className="flex flex-wrap gap-1">
                {['payment.success', 'lead.created', 'enrollment.created'].map(ev => (
                  <span key={ev} className="text-[10px] bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded text-slate-600">{ev}</span>
                ))}
              </div>
            </div>
        ...
        <Card className="hover:shadow-md transition-shadow relative overflow-hidden flex flex-col justify-between border-slate-200/80">
          ...
          <CardHeader className="pb-3">
            <div className="flex items-start justify-between">
              <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-slate-100 border border-slate-300 text-slate-800 dark:bg-background dark:border-slate-900">
          ...
          <CardContent className="pt-0 space-y-4">
            <div className="space-y-1">
              <label className="text-[11px] font-semibold text-slate-500 block">Production API Key</label>
              <div className="flex items-center gap-1.5 bg-slate-50 dark:bg-slate-900 border rounded-lg p-2 text-xs">
                <Lock className="h-3.5 w-3.5 text-slate-400 shrink-0" />
                <span className="font-mono flex-1 truncate text-slate-700 dark:text-slate-300">
                  {apiKey
                    ? (showKey ? apiKey : apiKey.replace(/(.{8}).+(.{4})/, '$1••••••••••••$2'))
                    : <span className="text-slate-400 italic">No key generated</span>
                  }
                </span>
                {apiKey && (
                  <button type="button" onClick={() => setShowKey(!showKey)} className="text-slate-400 hover:text-slate-600">
            ...
            <div className="flex gap-2">
              ...
              <Button size="sm" className="flex-1 text-xs bg-slate-800 hover:bg-slate-900 text-white" onClick={handleGenerateKey} disabled={busy.apikey}>
      ...
      {activeModal === 'google_config' && (
        <Modal onClose={closeModal}>
          ...
          <form onSubmit={handleSaveGoogleConfig} className="space-y-4">
            <div>
              <label className="text-xs font-semibold text-slate-500">Client ID *</label>
              <Input required value={googleClientId} onChange={e => setGoogleClientId(e.target.value)} placeholder="Google OAuth Client ID" />
            </div>
            <div>
              <label className="text-xs font-semibold text-slate-500">Client Secret *</label>
        ...
        <Modal onClose={closeModal}>
          ...
          <form onSubmit={handleCreateEvent} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div>
                <label className="text-xs font-semibold text-slate-500">Guest Email</label>
      ...
      {activeModal === 'zoom_manage' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto animate-in fade-in duration-200"
          ...
          <div className="bg-card border dark:border-slate-800 rounded-2xl w-full max-w-xl shadow-2xl p-6 my-8 max-h-[90vh] overflow-y-auto">
            ...
            <div className="max-h-48 overflow-y-auto mb-4 border rounded-lg bg-slate-50 dark:bg-slate-900">
              <table className="w-full text-xs text-left">
                ...
                <tbody className="divide-y divide-slate-200">
                  {busy.list_zoom && (
                    <tr><td colSpan={4} className="p-4 text-center text-slate-400"><Loader2 className="h-4 w-4 animate-spin inline mr-2" />Loading…</td></tr>
                  )}
                  {!busy.list_zoom && zoomMeetings.length === 0 && (
                    <tr><td colSpan={4} className="p-4 text-center text-slate-400">No meetings yet.</td></tr>
                  )}
                  {zoomMeetings.map(zm => (
                    <tr key={zm.id}>
                      <td className="p-2.5 font-medium truncate max-w-[160px]">{zm.topic}</td>
                      <td className="p-2.5 text-slate-500">{new Date(zm.time).toLocaleString()}</td>
                      <td className="p-2.5 text-center text-slate-500">{zm.duration}</td>
            ...
            <form onSubmit={handleCreateMeeting} className="border-t pt-4 space-y-3">
              <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400">New Meeting</h4>
              <div className="grid grid-cols-2 gap-2">
      ...
      {activeModal === 'cashfree_config' && (
        <Modal onClose={closeModal}>
          ...
          <form onSubmit={handleSaveCashfreeConfig} className="space-y-4">
            <div>
              <label className="text-xs font-semibold text-slate-500">App ID *</label>
            ...
            <div>
              <label className="text-xs font-semibold text-slate-500">Return URL *</label>
            ...
            <div>
              <label className="text-xs font-semibold text-slate-500">Notify URL *</label>
            ...
            <div className="flex items-center justify-between border-t pt-3">
              <div>
                <span className="text-xs font-semibold text-slate-700 block">Sandbox Mode</span>
                <span className="text-[10px] text-slate-400">Routes through Cashfree sandbox.</span>
              </div>
      ...
      {activeModal === 'cashfree_order' && (
        <Modal onClose={closeModal}>
          ...
          <form onSubmit={handleCreateOrder} className="space-y-3">
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="text-[11px] font-semibold text-slate-500">Order ID (Optional)</label>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div>
                <label className="text-[11px] font-semibold text-slate-500">Reference / CRM ID</label>
      ...
      {activeModal === 'cashfree_txs' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto animate-in fade-in duration-200"
          ...
          <div className="bg-card border dark:border-slate-800 rounded-2xl w-full max-w-xl shadow-2xl p-6 my-8 max-h-[90vh] overflow-y-auto">
            ...
            <div className="border rounded-lg overflow-hidden bg-slate-50 dark:bg-slate-900 max-h-64 overflow-y-auto">
              <table className="w-full text-xs text-left">
                ...
                <tbody className="divide-y divide-slate-200">
                  {busy.list_cf && (
                    <tr><td colSpan={5} className="p-4 text-center text-slate-400"><Loader2 className="h-4 w-4 animate-spin inline mr-2" />Loading…</td></tr>
                  )}
          ...
          <div className="bg-card border dark:border-slate-800 rounded-2xl w-full max-w-xl shadow-2xl p-6 my-8 max-h-[90vh] overflow-y-auto">
            ...
            <div className="max-h-48 overflow-y-auto mb-4 border rounded-lg bg-slate-50 dark:bg-slate-900">
              <table className="w-full text-xs text-left">
                ...
                <tbody className="divide-y divide-slate-200">
                  {busy.list_wa && <tr><td colSpan={3} className="p-4 text-center text-slate-400"><Loader2 className="h-4 w-4 animate-spin inline mr-2" />Loading…</td></tr>}
                  {!busy.list_wa && whatsappTemplates.length === 0 && <tr><td colSpan={3} className="p-4 text-center text-slate-400">No templates.</td></tr>}
            ...
            <form onSubmit={handleCreateTemplate} className="border-t pt-4 space-y-3">
              <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400">New Template</h4>
              <div className="grid grid-cols-2 gap-2">
      ...
      {activeModal === 'zapier_setup' && (
        <Modal onClose={closeModal}>
          ...
          <div className="text-xs text-slate-500 bg-slate-50 dark:bg-slate-900 p-3 rounded-lg space-y-1.5 border mt-4">
            <span className="font-bold text-slate-700 dark:text-slate-300 block">Quick Steps</span>
      ...
      {activeModal === 'webhooks_list' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto animate-in fade-in duration-200"
          onClick={e => { if (e.target === e.currentTarget) closeModal(); }}>
          <div className="bg-card border dark:border-slate-800 rounded-2xl w-full max-w-2xl shadow-2xl p-6 my-8 max-h-[90vh] overflow-y-auto">
            <h3 className="text-lg font-bold mb-1 flex items-center gap-2"><Webhook className="h-5 w-5 text-slate-600" /> Outbound Webhooks</h3>
            ...
            <div className="max-h-48 overflow-y-auto mb-4 border rounded-lg bg-slate-50 dark:bg-slate-900">
              <table className="w-full text-xs text-left">
                ...
                <tbody className="divide-y divide-slate-200">
                  {busy.list_wh && <tr><td colSpan={4} className="p-4 text-center text-slate-400"><Loader2 className="h-4 w-4 animate-spin inline mr-2" />Loading…</td></tr>}
                  {!busy.list_wh && webhooks.length === 0 && <tr><td colSpan={4} className="p-4 text-center text-slate-400">No webhooks registered.</td></tr>}
                  {webhooks.map(wh => (
                    <tr key={wh.id}>
                      ...
                      <td className="p-2.5">
                        <div className="flex flex-wrap gap-1">
                          {wh.events.map(ev => (
                            <span key={ev} className="bg-slate-200/60 dark:bg-slate-800 text-[9px] px-1 py-0.5 rounded text-muted-foreground">{ev}</span>
                          ))}
                        </div>
                      </td>
                      <td className="p-2.5 text-center">
                        <span className={`px-1.5 py-0.5 text-[10px] rounded font-semibold ${wh.active ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600'}`}>
            ...
            <form onSubmit={handleAddWebhook} className="border-t pt-4 space-y-3">
              <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400">Register New Endpoint</h4>
              ...
              <div>
                <label className="text-[11px] font-semibold text-slate-400 block mb-1">Trigger Events</label>
                <div className="flex flex-wrap gap-2 text-xs">
                  {['payment.success', 'payment.failed', 'lead.created', 'enrollment.created'].map(ev => (
                    <label key={ev} className="flex items-center gap-1.5 bg-slate-50 dark:bg-slate-800 p-1.5 rounded border border-slate-200 cursor-pointer">
              ...
              <div className="flex gap-2 justify-end pt-1">
                <Button type="button" variant="outline" size="sm" onClick={closeModal}>Close</Button>
                <Button type="submit" size="sm" className="bg-slate-800 hover:bg-slate-900 text-white" disabled={busy.webhook}>
