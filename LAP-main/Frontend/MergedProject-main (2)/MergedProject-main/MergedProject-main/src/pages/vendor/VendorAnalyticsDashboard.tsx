...
const StatCard = ({ title, value, change, isPositive, icon: Icon, delay }: StatCardProps) => (
  <motion.div
    ...
    className="bg-card border border-border rounded-xl p-5 relative overflow-hidden group shadow-sm"
  >
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <motion.div
          ...
          className="bg-card border border-border rounded-xl p-4 sm:p-6 lg:col-span-2 shadow-sm"
        >
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            ...
            <div className="flex gap-2 w-full sm:w-auto text-xs">
              <select 
                className="bg-background border border-border text-foreground text-xs rounded-lg focus:ring-cyan-500 focus:border-cyan-500 block px-3 py-1.5 flex-1 sm:flex-none"
              ...
              {!['Last 6 months', 'This Year', 'Last Year'].includes(spendFilter) && (
                <input 
                  ...
                  placeholder="YYYY"
                  className="bg-background border border-border text-foreground text-xs rounded-lg focus:ring-cyan-500 focus:border-cyan-500 block px-3 py-1.5 w-24"
                  value={spendFilter}
          ...
          <div className="h-72 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={data.spendData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <XAxis dataKey="name" stroke="var(--muted-foreground)" axisLine={false} tickLine={false} tick={{ fontSize: 12 }} dy={10} />
                <YAxis stroke="var(--muted-foreground)" axisLine={false} tickLine={false} tick={{ fontSize: 12 }} tickFormatter={(val) => `$${val/1000}k`} />
                <Tooltip 
          ...
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data.vendorData} layout="vertical" margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                ...
                <XAxis type="number" stroke="var(--muted-foreground)" axisLine={false} tickLine={false} tick={{ fontSize: 12 }} />
                <YAxis dataKey="name" type="category" stroke="var(--foreground)" axisLine={false} tickLine={false} tick={{ fontSize: 12 }} width={80} />
                <Tooltip 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          <div className="flex justify-between items-center mb-4">
            ...
            <button className="text-cyan-600 dark:text-cyan-400 text-sm hover:underline cursor-pointer">View All</button>
          </div>
          <div className="space-y-3">
            {filteredAlerts.length > 0 ? (
              filteredAlerts.map((alert, i) => (
                <div key={i} className="flex items-start p-3 bg-muted/40 rounded-lg border border-border">
            ...
            {filteredActivities.length > 0 ? (
              filteredActivities.map((act, i) => (
                <div key={i} className="flex justify-between items-start">
                  <div className="relative pl-4 before:absolute before:left-0 before:top-2 before:w-1.5 before:h-1.5 before:bg-muted-foreground before:rounded-full after:absolute after:left-[2.5px] after:top-4 after:w-0.5 after:h-full after:bg-border last:after:hidden">
